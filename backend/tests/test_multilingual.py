...
async def test_4_english_to_gujarati() -> None:
    async with (
        ...
        result = await session.run(
            user_input="I have been feeling weak since yesterday."
        )
        msg_assert = await _get_agent_response_assert(result)
        ...
        await msg_assert.judge(
            judge,
            intent="The agent MUST immediately switch to Gujarati script. It MUST NOT say 'I will speak Gujarati now' or announce the switch. It MUST NOT speak Hindi.",
        )
...
async def test_5_gujarati_to_english() -> None:
    async with (
        ...
        await msg_assert.judge(
            judge,
            intent="The agent MUST immediately switch to English. It MUST NOT announce the switch.",
        )
...
async def test_6_gujarati_to_hindi() -> None:
    async with (
        ...
        await msg_assert.judge(
            judge,
            intent="The agent MUST immediately switch to Hindi (Devanagari script). It MUST NOT announce the switch. It MUST NOT continue in Gujarati.",
        )
...
async def test_7_hindi_to_gujarati() -> None:
    async with (
        ...
        await msg_assert.judge(
            judge,
            intent="The agent MUST immediately switch to Gujarati script. It MUST NOT announce the switch. It MUST NOT continue in Hindi.",
        )
...
async def test_10_rapid_switching() -> None:
    async with (
        ...
        msg_assert = await _get_agent_response_assert(result)
        await msg_assert.judge(
            judge, intent="Respond in English. Must refuse prescription."
        )
